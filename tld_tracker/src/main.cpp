...
void Main::imageReceivedCB(const sensor_msgs::ImageConstPtr & msg)
{
  ...
  try
  {
    ...
    if(img_buffer_ptr->image.cols/2!=center_camera_view.x || img_buffer_ptr->image.cols/2!=center_camera_view.x ){
        center_camera_view.x= img_buffer_ptr->image.cols/2;
        center_camera_view.y= img_buffer_ptr->image.rows/2;
        geometry_msgs::Point p;
        p.x = center_camera_view.x;
        p.y = center_camera_view.y;
        pub_center_camera.publish(p);
      }
  }
